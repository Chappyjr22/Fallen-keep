...
export async function handleApi(request,env){const user=await accountKey(request.headers);if(!user)return json({error:'Your ChatGPT session is unavailable. Close and reopen the game.'},401);const db=database(env),url=new URL(request.url);
 ...
 if(url.pathname==='/api/purchase'){
  ...
  if(!changed)return json({error:'Your balance or upgrade changed. Please try again.',profile:await profile(db,user)},409);
  return json({profile:await profile(db,user)});
 }
 if(url.pathname==='/api/run/start'){
  ...
  return json({runId:run.id,mapId:run.mapId,bonuses:JSON.parse(run.bonuses),profile:p});
 }
 if(url.pathname==='/api/run/checkpoint'){
  ...
  return json({profile:await profile(db,user),saved:true});
 }
